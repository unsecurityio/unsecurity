...
import unsecurity.hlinx.HLinx._
...
object Main extends IOApp {
  ...
  val post2 =
    unsecure(
      Endpoint(
        ...
        Method.POST,
        Root / "does" / "this" / "work",
        Accepts.jsonWithMediaType[Fjon](
  ...
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    new Server[IO](port = 8088,
                   host = "0.0.0.0",
                   httpExecutionContext = scala.concurrent.ExecutionContext.Implicits.global)
      .serve(
        List(
          helloWorld,
          collidingHello,
