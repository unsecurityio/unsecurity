...
class ContentTypeMatcherTest extends UnsecurityTestSuite {
  ...
  val invalidMediaType = Request[Id](method = Method.POST, uri = uri"/whatever", headers = Headers.of(Header("content-type", "foobar")))
  val unsupportedMediaType = Request[Id](method = Method.POST, uri = uri"/whatever", headers = Headers.of(Header("content-type", "application/foobar")))
  val supportedMediaType = Request[Id](method = Method.POST, uri = uri"/whatever", headers = Headers.of(Header("content-type", "application/fjon")))
  ...
  test("invalidMediaType") {
    ...
    val value = methodMatchDirective.run(invalidMediaType).where{
      ...
      && r.contentType.exists(_ == problemJsonContentType) => Ok
    }
  }
