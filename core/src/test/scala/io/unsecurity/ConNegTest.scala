...
class ConNegTest extends FlatSpec with Matchers{
  ...
  "Invalid content type" should "return Invalid Media-Type error" in {
    val req = Request[IO](method = Method.GET, uri = uri"/whatever" , headers = Headers(Header("content-type", "foobar") :: Nil))
  ...
  "Unsupported content-type" should "return Usupported Media-Type error" in {
    val req = Request[IO](method = Method.GET, uri = uri"/whatever" , headers = Headers(Header("content-type", "application/foobar") :: Nil))
    val result = Unsecurity.validateContentType(req, supportedContentTypes)
  ...
  "Supported content-type" should "return MediaType" in {
    val req = Request[IO](method = Method.GET, uri = uri"/whatever" , headers = Headers(Header("content-type", "application/vnd.testing+json") :: Nil))
    val result = Unsecurity.validateContentType(req, supportedContentTypes)
