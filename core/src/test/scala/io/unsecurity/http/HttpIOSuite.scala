...
class HttpIOSuite extends IOSuite {
  ...
  def server(httpRoute: HttpRoutes[IO], routes: Complete*): Fixture[Server] = {
    server(
      Server
        .toHttpRoutes(
          routes: _*
  ...
  def server(httpRoute: HttpRoutes[IO]): Fixture[Server] = suiteResourceFixture(
  ...
  def server(routes: Complete*): Fixture[Server] =
    server(
      Server
        .toHttpRoutes(
          routes: _*
